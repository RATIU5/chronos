if ! command -v limine &>/dev/null; then
	...
	if ! grep -Eq '^HOOKS=.*plymouth' /etc/mkinitcpio.conf; then
		...
		# Add plymouth to HOOKS array afer 'base udev' or 'base systemd'
	...
	if [ -d "/boot/loader/entries" ]; then
		...
		if ! grep -q splash /etc/cmdline.d/*.conf; then
			# Need a splash, create the chronos file
			echo "splash" | sudo tee -a /etc/cmdline.d/chronos.conf
		fi
	elif [ -f "etc/kernel/cmdline" ]; then
		# Alternative UKI kernel cmdline location
		echo "Detected a UKI setup"
