...
if [ "$(plymouth-set-default-theme)" != "chronos" ]; then
  sudo cp -r "$HOME/.local/share/chronos/default/plymouth" /usr/share/plymouth/themes/chronos/
  sudo plymouth-set-default-theme chronos
fi
...
if [ ! -f /etc/systemd/system/chronos-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/chronos-seamless-login.service
[Unit]
Description=Chronos Seamless Auto-Login
Documentation=https://github.com/RATIU5/chronos
...
# Enable chronos-seamless-login.service only if not already enabled
if ! systemctl is-enabled chronos-seamless-login.service | grep -q enabled; then
  sudo systemctl enable chronos-seamless-login.service
fi
