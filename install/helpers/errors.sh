...
show_log_tail() {
	if [[ -f $CHRONOS_INSTALL_LOG_FILE ]]; then
		local log_lines=$(($TERM_HEIGHT - $LOGO_HEIGHT - 35))
		local max_line_width=$((LOGO_WIDTH - 4))

		tail -n $log_lines "$CHRONOS_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
show_failed_command_or_script() {
	if [[ -n ${CURRENT_SCRIPT:-} ]]; then
		gum style "Failed script: $CURRENT_SCRIPT"
	else
		# Truncate long command lines to fit in display
...
catch_errors() {
  ...
  # Store exit code immediately before it gets overwritten
	...
	gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Chronos installation stopped!"
	show_log_tail
	...
	while true; do
	  ...
	  # Add upload option if internet is available
		...
		case "$choice" in
		"Retry installation")
			bash ~/.local/share/chronos/install.sh
			break
			;;
		"View full log")
			if command -v less &>/dev/null; then
				less "$CHRONOS_INSTALL_LOG_FILE"
			else
				tail "$CHRONOS_INSTALL_LOG_FILE"
			fi
			;;
		"Upload log for support")
			chronos_upload_install_log
			;;
		"Exit")
			exit 1
			;;
